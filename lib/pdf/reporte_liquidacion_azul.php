...
while ($monedas=mysql_fetch_array($record_monedas)){ 
			  ...
			  $query_asientos="select id_caja,fecha,nro_recibo_factura,emitido_por,cuenta,signo,nombre,descripcion,concepto,monto,asignar_a,apellido,nombre from 
				tts_caja_azul inner join tts_cuentas on tts_caja_azul.cuenta=tts_cuentas.id_cuenta
				inner join personas on tts_caja_azul.asignar_a=personas.id_persona
				where  
				liquidado='$id_liquidacion'
		...
		if(mysql_num_rows($record_asientos)>0){
				...
				$query_ultima_liquidacion="select liquidacion,moneda,total from tts_liquidaciones 
				where persona='$persona_liquidada' and moneda='".$monedas["id_moneda"]."' order by liquidacion desc";
		
				$record_ultima_liquidacion=mysql_query($query_ultima_liquidacion,$link);
				
				if(mysql_num_rows($record_ultima_liquidacion)>1){
					$saldo_inicial=mysql_result($record_ultima_liquidacion,1,"total");
				}
