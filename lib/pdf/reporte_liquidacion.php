...
$query_persona="SELECT apellido, nombre, oficina, liquidacion
FROM tts_liquidaciones
INNER JOIN personas ON tts_liquidaciones.persona = personas.id_persona
...
$query_cuentas="select id_cuenta,codigo,descripcion,signo_liquidacion from tts_cuentas";
...
$query_liquidacion="select azul  from tts_liquidaciones where id_liquidacion='$id_liquidacion'";
$record_liquidacion=mysql_query($query_liquidacion,$link);
$azul=mysql_result($record_liquidacion,0,"azul");
...
//--------ENCABEZADO------------------------------
// SI ES LIQUIDACION AZUL, NO DEBE MOSTRAR EL LOGO
...
while ($monedas=mysql_fetch_array($record_monedas)){ 
			  ...
			  $query_asientos="select id_caja,fecha,nro_recibo_factura,emitido_por,cuenta,signo_liquidacion,nombre,descripcion,concepto,monto,asignar_a,apellido,nombre from 
				...
				and moneda= '".$monedas["id_moneda"]."' order by fecha";
		...
		if(mysql_num_rows($record_asientos)>0){
					...
					while($datatmp = mysql_fetch_assoc($record_asientos)) {
						...
						if($datatmp["signo_liquidacion"]==1){
						...
						if(is_null($datatmp["concepto"])){
						$datatmp["concepto"]=$datatmp["descripcion"];
						}
